...
req = requests.Session()  # cookie保持


class RemainderTicket(object):
    # ********  余票查询功能  ********* #
    def __init__(self):
        with open(r'city_code.txt', 'r') as file:
            self.CITY_CODE = eval(file.read())

    # ********  查询函数  ********* #
    def queryTicket(self, from_station, to_station, train_date):
        url = 'https://kyfw.12306.cn/otn/leftTicket/query?leftTicketDTO.train_date={}&leftTicketDTO.from_station={}&leftTicketDTO.to_station={}&purpose_codes=ADULT'.format(
            train_date, self.CITY_CODE[from_station], self.CITY_CODE[to_station])
        ...
        try:
            ...
            if result == []:
                print('对不起，没有查询到符合当前条件的列车')
            else:
                print('  ' + '2019-09-19' + '      ' + '北京' + '—' + '天津' + '        ' + '列车信息时刻表')
                print('*--------------------------------------------------*')

                n = 0  # 用于给车次编号，方便购买
                seat = {21: '高级软卧', 23: '软卧', 24: '软座', 26: '硬卧', 28: '硬座', 29: '无座', 30: '二等座', 31: '一等座', 32: '商务座',
                        33: '动卧'}  # 座次信息
                for i in result:
                    '''
                    打印所有车次信息
                    下标信息：{3:'车次',8:'出发时间',9:'到达时间',10:'历时'}
                    '''
                    info = i.split('|')
                    if info[0] != '' and info[0] != 'null':
                        print(str(n) + '  ' + info[3] + '    ', end='')
                        print('|' + '出发时间：' + info[8] + '|' + '到达时间：' + info[9] + '|' + '历时：' + info[10] + '|', end='')

                        for k in seat.keys():
                            if info[k] != '' and info[k] != '无':
                                print(seat[k] + ':' + info[k] + '|', end='')
                        n += 1
                        print('\n')

        except:
            print('您输入的查询信息有误，请重新输入！')
...
class Order(object):
    ...
    def order(self, result, train_number, from_station, to_station, date):
        ...
        form = {
            ...
            'train_date': date,  # 出发日期(2018-07-08)
        ...
        if html_query['data']['order_total_number'] == '':
            ...
            for i in data:
                ...
                for j in ticket_list:
                    ...
                    date = j['start_train_date_page']  # 乘车日期+时间 (2018-07-08 09:23)
...
def order():
    ...
    date = input('请输入您要购票的乘车日期(例:2018-07-06):')
    # 余票查询
    query = RemainderTicket()
    result = query.queryTicket(from_station, to_station, date)
    # 开始订票
    login = Login()
    # 填写验证码
...
def cancelticket():
    ...
    startdate = input('请输入查询起始日期(如:2018-07-16):')
    enddate = input('请输入查询最终日期(如:2018-07-16):')
